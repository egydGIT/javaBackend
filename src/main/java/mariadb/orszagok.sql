...
/*
A Föld országainak adatait tartalmazó adatbázis struktúrája a következő:
...
SELECT `nepesseg` FROM `orszagok` WHERE `orszag` = 'Japán'; 

--      9.  Mely országok népsűrűsége nagyobb 500 fő/km2-nél? 										?
SELECT `nepesseg`, `orszag` FROM `orszagok` WHERE `nepesseg` > 500;

--     10.  Melyek a szigetországok?
SELECT `orszag`, `foldr_hely` FROM `orszagok` where `foldr_hely` LIKE '%-sziget%';

--     11.  Melyik a első hat legnagyobb területű ország?
SELECT `terulet`, `orszag` FROM `orszagok` ORDER BY `terulet` DESC LIMIT 3;  

--     12.  Melyik három ország területe a legkisebb?
SELECT `terulet`, `orszag` FROM `orszagok` ORDER BY `terulet` ASC LIMIT 3;  
...
SELECT COUNT(`autojel`) FROM `orszagok` WHERE `autojel` LIKE '';

-- 14.  Hány országban nem 100 a váltószáma az aprópénznek? 
SELECT COUNT(`valtopenz`) FROM `orszagok` WHERE `valtopenz` LIKE '100 %';		-- kivonni? 
SELECT COUNT(`orszag`) FROM  `orszagok`;

-- 15.  Hány országnak nagyobb a népessége, mint Romániának?
SELECT count(orszag) FROM `orszagok` WHERE nepesseg > (SELECT nepesseg FROM `orszagok` WHERE orszag = 'Románia');

-- 16.  Mennyi Európa népessége és mennyi ország van Európában?
SELECT COUNT(`orszag`), SUM(nepesseg) FROM `orszagok` WHERE foldr_hely LIKE '%Európa%';

-- 17.  Mennyi olyan ország van, aminek a területe kisebb, mint Törökország
SELECT COUNT(orszag) FROM `orszagok` WHERE terulet < (SELECT terulet FROM `orszagok` WHERE orszag = 'Törökország');
​
...
SELECT orszag, terulet FROM `orszagok` WHERE terulet = 
	(SELECT MAX(`terulet`) FROM `orszagok` 
